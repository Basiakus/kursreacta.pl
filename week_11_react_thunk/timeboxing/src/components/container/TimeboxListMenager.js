...
import { 
   ...
   addTimebox, 
   //deleteTimebox, 
   deleteTimeboxRemotely,
   //updateTimebox,
   //timeboxEditStop,
   editingTimeboxRemotely 
...
function TimeboxListMenager() {
   ...
   useEffect(() => {
      dispatch(fetchAllTimeboxes(context.accessToken))
   ...
   const renderTimebox = (timebox) => {
      const onUpdate = (updatedTimebox) => {
         dispatch(editingTimeboxRemotely(timebox, updatedTimebox, context.accessToken))
      }
      const onDelete = () => {
         dispatch(deleteTimeboxRemotely(timebox, context.accessToken))
      }
